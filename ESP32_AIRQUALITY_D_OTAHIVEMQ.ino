...
const char* mqttClientId = "ESP32_D_Airmon";

const char* SENSOR_TOPIC = "sensor/airquality/D";
const char* OTA_TOPIC    = "esp32/D/ota";
const char* OTA_STATUS_TOPIC = "esp32/D/ota/status";
...
void mqttCallback(char* topic, byte* payload, unsigned int length){
  ...
  if(String(topic)==OTA_TOPIC){
    ...
    if(jd.containsKey("cmd") && String(jd["cmd"])=="update" && jd.containsKey("url") && jd.containsKey("token")){
      if(String(jd["token"])!=OTA_SECRET){ publishOtaStatus("invalid_token"); return; }
      performOta(String(jd["url"]).c_str());
    }
  }
}
...
void loop(){
  ...
  if(now-lastPublish>=PUBLISH_INTERVAL){
    ...
    doc["dev"]="D"; doc["version"]=FIRMWARE_VERSION;
    doc["temp"]=environment.getTemperature(TEMP_C); // hanya ini diganti
    doc["hum"]=environment.getHumidity();           // hanya ini diganti
    //doc["press"]=ads.readADC_SingleEnded(0)*0.1875/1000.0; // tetap seperti lama
    doc["press"]= environment.getAtmospherePressure(HPA);
    ...
    Serial.println(environment.getTemperature(TEMP_C));
  }
  delay(10);
}
