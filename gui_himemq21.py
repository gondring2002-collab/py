...
relay1 = ttk.LabelFrame(ctrl,text="Relay 1"); relay1.pack(fill="x", pady=5)
ttk.Button(relay1,text="Relay ON 1",command=lambda: send_relay("ON")).pack(fill="x", pady=2)
ttk.Button(relay1,text="Relay OFF 1",command=lambda: send_relay("OFF")).pack(fill="x", pady=2)
...
combo = ttk.Combobox(ctrl, textvariable=combo_var, values=["Mode 1","Mode 2","Mode 3"], state="readonly")
combo.current(0)
combo.pack(pady=2)
ttk.Button(ctrl, text="Kirim Mode", command=lambda: send_mode(combo_var.get())).pack(fill="x", pady=2)
