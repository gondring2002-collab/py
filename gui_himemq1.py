...
def start_mqtt():
    client = mqtt.Client(client_id="GUI_C")
...
def save_now_to_firebase():
    ...
    try:
        ...
        thread_safe_log("Saved manually to Firebase")
    except Exception as e:
...
def auto_save_loop():
    while True:
        ...
        sleep_sec = (next_hour - datetime.datetime.now()).total_seconds()
...
history_tree.heading("ts", text="Timestamp")
history_tree.heading("cmd", text="Command")
...
def plot_graph_popup():
    ...
    ttk.Combobox(popup,textvariable=sensor_var,values=["all","temp","hum","co2","pm25"],state="readonly").pack()
