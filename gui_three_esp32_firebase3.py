...
def save_now_to_firebase():
    ...
    try:
        ...
        if latest_A:
            ref_A.child(now).set(latest_A)
        if latest_C:
            ref_C.child(now).set(latest_C)
        messagebox.showinfo("Firebase", "Saved")
        log_tab1("Saved manually to Firebase")
    except Exception as e:
        messagebox.showerror("Firebase", str(e))

def load_history_from_firebase(dev):
    if not FIREBASE_AVAILABLE:
        messagebox.showwarning("Firebase", "Not configured")
        return
    ref = ref_A if dev == "A" else ref_C
    data = ref.get()
    for i in fb_tree.get_children():
        fb_tree.delete(i)
    if not data:
        messagebox.showinfo("Firebase", "No data")
        return
    for ts, rec in sorted(data.items()):
        fb_tree.insert("", "end",
            values=(ts, rec.get("temp",""), rec.get("hum",""),
                    rec.get("co2",""), rec.get("pm25","")))
    log_tab1(f"Loaded history {dev}")
...
ttk.Button(top2, text="SAVE NOW", command=save_now_to_firebase).pack(side="left", padx=10)
...
def run_sequence():
    def thread_func():
        ...
        log4("Stepper mundur")
        time.sleep(5)  # diam 5 detik
        send_relay("on")
        log4("Relay ON selama 1 menit")
        for i in range(60):
            if latest_A:
                log4(f"ESP32-A: {latest_A}")
            if latest_C:
                log4(f"ESP32-C: {latest_C}")
            time.sleep(1)
        send_relay("off")
        log4("Relay OFF")
...
ttk.Button(tab4, text="Jalankan", command=run_sequence, width=10).pack(pady=5)

# ---------------- TAB 2 AUTO SAVE PER JAM ----------------
def schedule_hour_save():
    now = datetime.datetime.now()
    next_hour = (now + datetime.timedelta(hours=1)).replace(minute=0, second=0, microsecond=0)
    seconds_until_next_hour = (next_hour - now).total_seconds()
    log4(f"Menunggu auto-save jam berikutnya: {next_hour.strftime('%H:%M:%S')}")
    root.after(int(seconds_until_next_hour * 1000), start_hour_sequence)

def start_hour_sequence():
    now = datetime.datetime.now()
    if now.hour == 12:  # skip jam 12 siang
        log4("Lewat jam 12 siang, skip auto-save")
        schedule_hour_save()
        return
    ...
    def take_10_samples():
        for i in range(10):
            ...
            msg = f"Auto-save {i+1}/10 at {ts}"
            log_tab1(msg)
            log4(msg)
            time.sleep(60)
        schedule_hour_save()

    threading.Thread(target=take_10_samples, daemon=True).start()

# ---------------- START ----------------
root.after(1000, schedule_hour_save)
