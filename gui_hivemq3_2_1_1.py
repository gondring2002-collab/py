...
TOTAL_WEEKS = 3          # total siklus minggu
last_trigger_time = None
start_week_key = None        
current_week = 0      # minggu berjalan 
# ---------------- Firebase init ----------------
ref_A = ref_C = None
...
def send_stepper_auto(topic, direction, timeout=30):
    ...
    #        continue
    #
    #    if topic == TOPIC_STEP1:
    #        if direction == "backward" and status.get('s1_lim') == 0:
    #            log_tab4.insert("end", "Stepper1 BACKWARD hit limit\n")
    #            client.publish(topic, "stop")
    #            return

    #    if topic == TOPIC_STEP2:
    #        if direction == "backward" and status.get('s2_lim') == 0:
    #            log_tab4.insert("end", "Stepper2 BACKWARD hit limit\n")
    #            client.publish(topic, "stop")
    #           return
    #
    #    time.sleep(0.1)
    ...
    #client.publish(topic, "stop")
    #log_tab4.insert("end", "Stepper timeout, stopped\n")
...
def run_sequence3():
    ...
    def worker():
        ...
        send_stepper_auto(TOPIC_STEP1, "backward")
        wait_60s_with_counter()
        log_tab4.insert("end", "STEPPER MAJU\n")
        send_stepper_auto(TOPIC_STEP1, "forward")
        wait_60s_with_counter()
        #Time.sleep(5)
        log_tab4.insert("end", "STEPPER HOMING FINISH\n")
        #wait_60s_with_counter()
...
def schedule_loop1():
    ...
    while True:
        ...
        target_day = DAY_MAP.get(schedule_day_var.get())
        target_hour = int(schedule_hour_var.get())
        target_minute = int(schedule_minute_var.get())
        ...
        if (
            ...
            if last_schedule_run_date != today and not auto_running:
                last_schedule_run_date = today
                root.after(
                    0,
                    lambda:
                        log_tab4.insert(
...
def scheduler_loop2():
    ...
    while True:
        ...
        selected_month = month_map[schedule_month_var.get()]
        selected_day = day_map[schedule_day_var.get()]
        selected_hour = int(schedule_hour_var.get())
        selected_minute = int(schedule_minute_var.get())

        if (
            now.month == selected_month and
            now.weekday() == selected_day and
            now.hour == selected_hour and
            now.minute == selected_minute
        ):
            current_key = f"{now.year}-{now.month}-{now.day}-{now.hour}-{now.minute}"

            if last_trigger_time != current_key:
                last_trigger_time = current_key

                print("=== AUTO START ===")

                # panggil fungsi auto kamu di sini
                threading.Thread(target=auto_start_process).start()
...
def scheduler_loop3():
    ...
    while True:
        ...
        if (
            now.weekday() == day_map[schedule_day_var.get()] and
            now.hour == int(schedule_hour_var.get()) and
            now.minute == int(schedule_minute_var.get())
        ):
            year, week, _ = now.isocalendar()
            current_key = f"{year}-W{week}"
            ...
            if last_trigger_time != current_key:
                ...
                log_tab4.insert(
                    "end",
                    f"AUTO START {now.strftime('%A %H:%M')}\n"
                )

                threading.Thread(
                    target=auto_start_process,
                    daemon=True
                ).start()
...
def scheduler_loop():
    global last_trigger_time, start_week_key
    ...
    while True:
        ...
        # HITUNG NEXT RUN
        # ===============================
        days_ahead = selected_day - now.weekday()
        if days_ahead < 0:
            days_ahead += 7
        ...
        if (
            ...
            year, week, _ = now.isocalendar()
            current_week_key = f"{year}-W{week}"

            if start_week_key is None:
                start_week_key = current_week_key

            if last_trigger_time != current_week_key:
                last_trigger_time = current_week_key

                # HITUNG MINGGU KE BERAPA
                start_year, start_week = start_week_key.split("-W")
                week_index = (
                    (int(year) - int(start_year)) * 52
                ...
                log_tab4.insert(
                    "end",
                    f"AUTO START {now.strftime('%A %H:%M')} "
                    f"({week_index} of {TOTAL_WEEKS} weeks)\n"
                )
        ...
        log_tab4.insert(
            "end",
            f"Next run: {next_run.strftime('%A %H:%M')} "
            f"({week_index} of {TOTAL_WEEKS} weeks)\n"
        )
...
def run_sequence4_1():
    ...
    selected_day = schedule_day_var.get()
    selected_hour = int(schedule_hour_var.get())
    target_minute = int(schedule_minute_var.get())
    log_tab4.insert(
        "end",
        f"\n⏳ Menunggu jadwal: {selected_day} {selected_hour:02d}:{target_minute:02d}\n"
    )
    ...
    def scheduler():
        ...
        # === JALANKAN SEQUENCE ===
        auto_running = True
        log_tab4.insert("end", "🚀 === AUTO SEQUENCE 4 START ===\n")
        #send_stepper_auto(TOPIC_STEP2, "backward")
        #send_stepper_auto(TOPIC_STEP1, "backward")
        #start_calibration()
        #wait_60s_with_counter()
        #wait_60s_with_counter()
        #send_stepper_auto(TOPIC_STEP2, "forward")
        #send_stepper_auto(TOPIC_STEP1, "forward")
        #wait_60s_with_counter()
        #stop_calibration()
        #log_tab4.insert("end", "=== AUTO START ===\n")
        ...
        send_stepper_auto(TOPIC_STEP2, "backward")
        wait_60s_with_counter()
        log_tab4.insert("end", "STEPPER MUNDUR\n")
        #send_stepper_auto(TOPIC_STEP1, "forward")
        #wait_60s_with_counter()
        #Time.sleep(5)
        log_tab4.insert("end", "STEPPER HOMING FINISH\n")
        ...
        log_tab4.insert("end", "✅ === AUTO SEQUENCE 4 END ===\n")
        auto_running = False
...
def run_sequence4():
    global auto_running, current_week
    ...
    def scheduler():
        ...
        while True:
            ...
            if current_day == selected_day:
                ...
                # Kalau sudah lewat hari ini, set target minggu depan
                if now >= target_time:
                    target_time += datetime.timedelta(days=7)
        ...
        while True:
            ...
            minutes = remaining // 60
            log_tab4.insert("end", f"⏳ Sisa waktu: {minutes} menit\n")
            log_tab4.see("end")
            time.sleep(60)
        ...
        log_tab4.insert(
            "end",
            f"🚀 AUTO START {selected_day} "
            f"{selected_hour:02d}:{target_minute:02d} "
            f"({current_week} of {TOTAL_WEEKS} weeks)\n"
        )
        log_tab4.see("end")
        ...
        if current_week < TOTAL_WEEKS:
            log_tab4.insert(
                "end",
                f"Next run: {selected_day} "
                f"{selected_hour:02d}:{target_minute:02d} "
                f"({current_week+1} of {TOTAL_WEEKS} weeks)\n"
            )
        else:
            log_tab4.insert("end", "✅ AUTO SEQUENCE COMPLETE for all weeks\n")
        log_tab4.see("end")
...
schedule_month_var = tk.StringVar(value="January")
schedule_day_var = tk.StringVar(root, value="Monday")
schedule_hour_var = tk.StringVar(root, value="08")
schedule_minute_var = tk.StringVar(root, value="00")
...
ttk.Label(control_frame, text="Bulan").pack(side="left")

month_combo = ttk.Combobox(
    control_frame,
    textvariable=schedule_month_var,
    values=[
...
month_combo.pack(side="left", padx=3)
# Combo Hari
...
day_combo.pack(side="left", padx=3)

# Combo Jam (24 jam)
hour_combo = ttk.Combobox(
    control_frame,
    textvariable=schedule_hour_var,
...
minute_combo = ttk.Combobox(
    control_frame,
    textvariable=schedule_minute_var,
    values=("10","20", "30", "40" ),
    state="readonly",
    width=5
)
...
ttk.Button(
    control_frame,
    text="JALANKAN AUTO SEQUENCE4",
    command=run_sequence4
